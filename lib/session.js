...
class MySQLSession {
  ...
  async getSession (userID, chatID) {
    ...
    if (!this.sessions[key]) {
      let [rows] = await this.client.query('SELECT `session` FROM `' + this.options.table + '` WHERE `user_id` = "' + userID + '" AND `chat_id` = "' + chatID + '"')
  ...
  async saveSession (userID, chatID, session) {
    if (!session || Object.keys(session).length === 0) {
      return await this.client.query('DELETE FROM `' + this.options.table + '` WHERE `user_id` = "' + userID + '" AND `chat_id` = "' + chatID + '"')
    }

    const sessionString = JSON.stringify(session)

    return await this.client.query('INSERT INTO `' + this.options.table + '` (`user_id`, `chat_id`, `session`) VALUE (\'' + userID + '\', \'' + chatID + '\', \'' + sessionString + '\') ON DUPLICATE KEY UPDATE `session` = \'' + sessionString + '\';')
  }
