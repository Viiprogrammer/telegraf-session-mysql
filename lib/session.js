  ...
  async getSession (userID, chatID) {
    ...
    if (!this.sessions[key]) {
      let [rows] = await this.client.query(`
        ...
        FROM ?
        WHERE user_id = ?
        AND chat_id = ?`, [this.options.table, userID, chatID])
  ...
  async saveSession (userID, chatID, session) {
    if (!session || Object.keys(session).length === 0) {
      return await this.client.query(`
        ...
        WHERE  user_id = ?
        AND chat_id = ?
      `, [this.options.table, userID, chatID])
    }

    const sessionString = JSON.stringify(session)

    return await this.client.query(`
        INSERT INTO ? (user_id, chat_id, session) value (?, ?, ?) ON DUPLICATE KEY UPDATE session = ?;
    `, [this.options.table, userID, chatID, sessionString])
  }
